...
load_pg_cfg() {
    IMAGE_NAME="${POSTGRES_DOCKER_IMAGE:-roc-postgres}"
    CONTAINER_NAME="${POSTGRES_DOCKER_CONTAINER:-roc-postgres}"
    HOST_PORT="${POSTGRES_DOCKER_HOST_PORT:-5432}"
    CONTAINER_PORT="${POSTGRES_CONTAINER_PORT:-5432}"
    VOLUME_NAME="${POSTGRES_DOCKER_VOLUME:-roc_postgres_data}"
    DOCKERFILE_PATH="${POSTGRES_DOCKERFILE:-docker/postgres/Dockerfile}"
...
delete_volume() {
    ...
    if docker volume ls --format '{{.Name}}' | grep -q "^${VOLUME_NAME}$"; then
        log_warn "删除数据卷（危险操作）: ${VOLUME_NAME}"
        docker volume rm "${VOLUME_NAME}"
        log_info "数据卷已删除: ${VOLUME_NAME}"
    else
        log_info "数据卷不存在: ${VOLUME_NAME}"
    fi
}
